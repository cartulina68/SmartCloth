...
export default function ProductoIndex({ productos, categorias, generos }: Props) {
                ...
                <tbody>
                  {productos.map(producto => (
                    <tr key={producto.id} className="border-t">
                      ...
                      <td className="p-2">{producto.genero?.genero}</td>
                      <td className="p-2">{Number(producto.precio).toFixed(2)}</td>
                      <td className="p-2 space-x-2">
                        <Button size="sm" variant="outline" onClick={() => openEdit(producto.id)}>Editar</Button>
